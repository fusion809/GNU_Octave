...
\begin{document}
	...
	If we set $m_{1r} = m_{2r} = b_{1r} = b_{2r} = b_{2b} = b_{1b} = 0$, $m_{2b}=m_2$ and $m_{1b} = m_1$, we get:
	
	\begin{align*}
		\ddot{\theta_2} &= \dfrac{m_1+m_2}{m_{2}^2 r_2(1+\cos^2{(\theta_1-\theta_2)}) +m_1m_2r_2}\left[-\dfrac{m_2 \cos{(\theta_1-\theta_2)}}{m_1+m_2}\left(m_2 r_2 \dot{\theta}_2^2 \sin{(\theta_1-\theta_2)}-g\cos{\theta_1}(m_1+m_2)\right)+m_2 (r_1 \dot{\theta}_1^2 \sin{(\theta_1-\theta_2)}-g\cos{\theta_2})\right]\\
		&= -\dfrac{m_2^2 r_2 \dot{\theta}_2^2 \cos{(\theta_1-\theta_2)}\sin{(\theta_1-\theta_2)}}{m_{2}^2 r_2(1+\cos^2{(\theta_1-\theta_2)}) +m_1m_2r_2} + \dfrac{m_2(m_1+m_2)g\cos{(\theta_1-\theta_2)\cos{\theta_1}}}{m_{2}^2 r_2(1+\cos^2{(\theta_1-\theta_2)}) +m_1m_2r_2} + \dfrac{m_2(m_1+m_2)(r_1\dot{\theta}_1^2\sin{(\theta_1-\theta_2)}-g\cos{\theta_2})}{m_{2}^2 r_2(1+\cos^2{(\theta_1-\theta_2)}) +m_1m_2r_2}
	\end{align*}
	
	Based on ChatGPT, $\ddot{\theta}_2$ should be:
	
	\begin{align*}
		\ddot{\theta}_2 &= \dfrac{1}{(m_1+m_2)m_2 r_1^2 r_2^2 - m_2^2r_1^2 r_2^2 \cos^2{(\theta_1-\theta_2)}} [-m_2 r_1 r_2 \cos{(\theta_1-\theta_2)}(-m_2r_1r_2 \dot{\theta}_2^2 \sin{(\theta_1-\theta_2)}-(m_1+m_2)gr_1\cos{\theta_1})+(m_1+m_2)r_1^2(m_2r_1r_2 \dot{\theta}_1^2 \sin{(\theta_1-\theta_2)}\\ 
		&- m_2gr_2\cos(\theta_2))] \\
		&= \dfrac{1}{m_1m_2r_2^2+m_2^2r_2^2\sin^2(\theta_1-\theta_2)}\left[m_2r_2\cos{(\theta_1-\theta_2)}(r_2 \dot{\theta}_2^2\sin{(\theta_1-\theta_2)}+(m_1+m_2)g\cos{\theta_1})+(m_1+m_2)(m_2r_1r_2 \dot{\theta}_1^2 \sin{(\theta_1-\theta_2)}- m_2gr_2\cos(\theta_2))\right]
	\end{align*}
	\end{landscape}
\end{document}